...
router.delete('/:id', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  try {
    ...
    await User.deactivate(req.params.id, reason || null);
    await AuditLog.log(req.session.user.id, 'DEACTIVATE_USER', 'users', req.params.id, { reason: reason || '' }, req.ip);
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.put('/:id/activate', requireAuth, requireRole('admin'), async (req, res) => {
  try {
    ...
    await AuditLog.log(req.session.user.id, 'ACTIVATE_USER', 'users', req.params.id, {}, req.ip);
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
