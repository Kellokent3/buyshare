...
// ── GET /api/requests/portfolio (investor portfolio with gain/loss) ───
router.get('/portfolio', requireAuth, requireRole('investor'), async (req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT pr.id, pr.quantity, pr.price_per_share as purchase_price,
              pr.total_amount, pr.status, pr.created_at,
              s.share_name, s.price_per_share as current_price, s.currency,
              b.name as bank_name
       FROM purchase_requests pr
       JOIN shares s ON pr.share_id = s.id
       JOIN banks b ON s.bank_id = b.id
       WHERE pr.investor_id = ? AND pr.status = 'approved'
    ...
    const portfolio = rows.map(r => {
      ...
      return { ...r, invested, current_value: currentValue, gain, gain_pct: gainPct };
    });
    res.json(portfolio);
