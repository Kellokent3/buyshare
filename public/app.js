'use strict';
// ════════════════════════════════════════════════════════════════
// BuyShare v2 – app.js (Complete, Bug-Free)
// Key fixes:
...
const S = {
  ...
  confirmCb:   null,
  notifTimer:  null,
  currentBuyShare: null,  // keep reference to share being bought
};
...
async function doLogin(e) {
  ...
  try {
    ...
    if (msg.includes('lock'))       errEl.textContent = t('account_locked');
    else if (msg.includes('init'))  errEl.textContent = '⏳ Server is initializing… wait 5s and retry.';
    else                            errEl.textContent = t('invalid_creds');
...
function buildSidebar() {
  const NAV = {
    ...
    investor:     [['dashboard','fa-chart-pie'],['shares','fa-layer-group'],['my-requests','fa-receipt']],
  };
  const nav = $('#sb-nav');
  nav.innerHTML = '';
...
function loadPage(p) {
  const MAP = { dashboard:loadDash, shares:loadShares, 'my-requests':loadMyReqs, 'manage-requests':loadMngReqs, users:loadUsers, reports:loadReports };
  if (MAP[p]) MAP[p]();
}
...
async function loadShares() {
  ...
  if (addBtn) addBtn.classList.toggle('hidden', S.user.role === 'investor');
...
function buildShareCard(s) {
  ...
  const acts = isInv
    ...
    : '<button class="btn btn-secondary btn-sm" data-action="edit-share" data-id="'+s.id+'"><i class="fas fa-edit"></i> '+t('edit')+'</button>'+
      (S.user.role==='admin' ? ' <button class="btn btn-danger btn-sm" data-action="del-share" data-id="'+s.id+'"><i class="fas fa-trash"></i></button>' : '');
...
function attachDelegates() {
  ...
  if (grid) grid.addEventListener('click', e => {
    ...
    else if (action==='del-share')  delShare(id);
...
function delShare(id) {
  ...
  S.confirmCb = async () => {
    try { await api('DELETE','/api/shares/'+id); closeMod('conf-ov'); toast(t('msg_share_del'),'in'); loadShares(); }
...
async function loadUsers() {
  ...
  users.forEach(u => {
    ...
    tr.innerHTML =
      ...
      '<td><span class="st '+(u.is_active?'st-active':'st-inactive')+'">'+(u.is_active?t('active'):t('inactive'))+'</span></td>'+
      '<td class="td-acts">'+
        '<button class="btn btn-secondary btn-sm" data-action="edit-user" data-id="'+u.id+'"><i class="fas fa-edit"></i></button>'+
        (sameId(u.id,S.user.id)?'':' <button class="btn btn-danger btn-sm" data-action="del-user" data-id="'+u.id+'"><i class="fas fa-ban"></i></button>')+
      '</td>';
...
function delUser(id) {
  ...
  $('#conf-msg').textContent = t('confirm_delete');
  S.confirmCb = async () => {
    try { await api('DELETE','/api/users/'+id); closeMod('conf-ov'); toast(t('msg_user_del'),'in'); loadUsers(); }
    catch(ex) { toast(ex.message,'er'); }
  };
  openMod('conf-ov');
}
